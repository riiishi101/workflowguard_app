...
describe('HubSpotBillingController', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [{ provide: HubSpotBillingService, useValue: service }],
    }).compile();
  ...
  it('should process webhook with valid signature', async () => {
    ...
    const req = {
      headers: { 'x-hubspot-signature': makeSignature(body) },
    } as any as Request;
    const res = {
  ...
  it('should reject webhook with invalid signature', async () => {
    const body = { portalId: '123', newPlanId: 'pro' };
    const req = {
      headers: { 'x-hubspot-signature': 'invalid' },
    } as any as Request;
    const res = {
      status: jest.fn().mockReturnThis(),
      json: jest.fn(),
  ...
  it('should return 500 if HUBSPOT_CLIENT_SECRET is not set', async () => {
    ...
    const body = { portalId: '123', newPlanId: 'pro' };
    const req = {
      headers: { 'x-hubspot-signature': 'anything' },
    } as any as Request;
    const res = {
      status: jest.fn().mockReturnThis(),
      json: jest.fn(),
    ...
    expect(res.json).toHaveBeenCalledWith({
      message: 'HUBSPOT_CLIENT_SECRET not set in environment',
    });
  });
});
