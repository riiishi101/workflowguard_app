...
export class AuthController {
  ...
  async initiateHubSpotOAuth(@Res() res: Response) {
    ...
    if (!clientId || !redirectUriRaw) {
      throw new Error(
        'HUBSPOT_CLIENT_ID and HUBSPOT_REDIRECT_URI must be set in environment variables',
      );
    }
    const redirectUri = encodeURIComponent(redirectUriRaw);
    const scopes = encodeURIComponent(
      'crm.objects.companies.read crm.objects.contacts.read crm.objects.deals.read crm.schemas.companies.read crm.schemas.contacts.read crm.schemas.deals.read oauth',
    );
  ...
  debugCookies(@Req() req: Request) {
    this.logger.log(
      'Debug cookies endpoint hit. Incoming cookies:',
      req.cookies,
    );
  ...
  async hubspotOAuthCallback(
    ...
    this.logger.log('-----------------------------');
    this.logger.log(
      `OAuth callback called with code: ${code ? 'present' : 'missing'}`,
    );
    this.logger.log(`OAuth callback - State parameter: ${state}`);
    this.logger.log(
      `OAuth callback - All query parameters: ${JSON.stringify(allQueryParams)}`,
    );
    this.logger.log(
      `OAuth callback - Request headers: ${JSON.stringify(req.headers)}`,
    );
    ...
    try {
      ...
      try {
        ...
        if (!frontendUrl)
          throw new Error('FRONTEND_URL must be set in environment variables');
        const errorMsg = encodeURIComponent(
          'Failed to exchange code for access token. Please try again or contact support.',
        );
      ...
      try {
        ...
        if (!frontendUrl)
          throw new Error('FRONTEND_URL must be set in environment variables');
        const errorMsg = encodeURIComponent(
          'Failed to fetch user info from HubSpot. Please try again or contact support.',
        );
      ...
      try {
        if (portalId) {
          await this.authService.updateUserHubspotPortalId(
            user.id,
            portalId.toString(),
          );
        }
        ...
        if (!frontendUrl)
          throw new Error('FRONTEND_URL must be set in environment variables');
        const errorMsg = encodeURIComponent(
          'Failed to update user tokens. Please try again or contact support.',
        );
      ...
      try {
        ...
        if (!frontendUrl)
          throw new Error('FRONTEND_URL must be set in environment variables');
        const errorMsg = encodeURIComponent(
          'Failed to generate authentication token. Please try again or contact support.',
        );
      ...
      try {
        ...
        if (!frontendUrl)
          throw new Error('FRONTEND_URL must be set in environment variables');
        const errorMsg = encodeURIComponent(
          'Failed to set authentication cookie. Please try again or contact support.',
        );
        return res.redirect(`${frontendUrl}/?oauth_error=${errorMsg}`);
      }
      ...
      if (!frontendUrl)
        throw new Error('FRONTEND_URL must be set in environment variables');
      const errorMsg = encodeURIComponent(
        'Unexpected error during HubSpot connection. Please try again or contact support.',
      );
      return res.redirect(`${frontendUrl}/?oauth_error=${errorMsg}`);
    }
  }
  ...
  async getHubspotContacts(
    ...
    try {
      ...
      if (!user || !user.hubspotAccessToken) {
        return res
          .status(404)
          .json({ message: 'User or HubSpot access token not found' });
      }
      ...
      return res.status(500).json({
        message: 'Failed to fetch contacts from HubSpot',
  ...
  async getHubspotCompanies(
    ...
    try {
      ...
      const accessToken =
        await this.authService.getValidHubspotAccessToken(user);
      const hubspotRes = await axios.get(
      ...
      return res.status(500).json({
        message: 'Failed to fetch companies from HubSpot',
        error: error.response?.data || error.message,
      });
    }
  }
  ...
  async handleHubspotDeauth(@Body() body: any, @Res() res: any) {
    ...
    if (!portalId) {
      return res
        .status(400)
        .json({ message: 'Missing portalId in request body' });
    }
    try {
      const result = await this.authService.handleHubspotDeauth(
        portalId,
        userId,
      );
      this.logger.log(
        `Received HubSpot deauthorization for portalId: ${portalId}, userId: ${userId}`,
      );
      return res.status(200).json(result);
    } catch (err) {
      this.logger.error('Error handling HubSpot deauthorization', err);
      return res.status(500).json({
        message: 'Failed to handle deauthorization',
        error: err.message,
      });
    }
  }
