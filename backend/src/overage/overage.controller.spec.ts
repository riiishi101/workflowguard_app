...
describe('OverageController', () => {
  ...
  beforeEach(async () => {
    ...
    hubspotBillingService = module.get<HubSpotBillingService>(HubSpotBillingService);
  });
  ...
  describe('findAll', () => {
    ...
    it('should return overages with period filters', async () => {
      const mockOverages = [{ id: '1', amount: 100 }];
      (overageService.findAll as jest.Mock).mockResolvedValue(mockOverages);
      const result = await controller.findAll(undefined, undefined, '2024-01-01', '2024-01-31');
      expect(result).toEqual(mockOverages);
      expect(overageService.findAll).toHaveBeenCalledWith({
  ...
  describe('findOne', () => {
    ...
    it('should throw HttpException when overage not found', async () => {
      ...
      await expect(controller.findOne('999')).rejects.toThrow('Overage not found');
    });
  });
  ...
  describe('bulkBillOverages', () => {
    it('should successfully bulk bill overages', async () => {
      ...
      (hubspotBillingService.reportOveragesToHubSpot as jest.Mock).mockResolvedValue(mockResults);
      
      const result = await controller.bulkBillOverages({ overageIds: ['1', '2'] });
    ...
    it('should throw HttpException when bulk billing fails', async () => {
      (hubspotBillingService.reportOveragesToHubSpot as jest.Mock).mockRejectedValue(
        new Error('HubSpot API error')
      );
      
      await expect(controller.bulkBillOverages({ overageIds: ['1'] })).rejects.toThrow(HttpException);
      await expect(controller.bulkBillOverages({ overageIds: ['1'] })).rejects.toThrow('Bulk billing failed: HubSpot API error');
    });
  });
  ...
  describe('getBillingStatus', () => {
    it('should return operational status when no errors', async () => {
      ...
      (overageService.getUnbilledOverages as jest.Mock).mockResolvedValue(mockUnbilledOverages);
  ...
  describe('getOverageSummary', () => {
    it('should return overage summary without date filters', async () => {
      ...
      (overageService.getOverageSummary as jest.Mock).mockResolvedValue(summary);
      const result = await controller.getOverageSummary();
      expect(result).toEqual(summary);
      expect(overageService.getOverageSummary).toHaveBeenCalledWith(undefined, undefined);
    });

    it('should return overage summary with date filters', async () => {
      const summary = { totalOverages: 50, unbilledOverages: 10 };
      (overageService.getOverageSummary as jest.Mock).mockResolvedValue(summary);
      const result = await controller.getOverageSummary('2024-01-01', '2024-01-31');
      expect(result).toEqual(summary);
  ...
  describe('getUnbilledOverages', () => {
    it('should return unbilled overages', async () => {
      ...
      (overageService.getUnbilledOverages as jest.Mock).mockResolvedValue(unbilled);
